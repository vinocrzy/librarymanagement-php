						...
						<?php
										...
										if($activity) {   
											...
											if($activity == 'updateMember'){
												...
												$query = mysqli_query($db,"SELECT * From members Where Id = '$uMemberId'");
												$result = mysqli_fetch_assoc($query);
												
												?>
												<h1 class="major">Update Member<br /></h1>
												<div class="updatearea">
													<form action="home.php" class="updateForm">
														<input class="form-control placeholder-no-fix" type="text" name="memberId" value=<?php echo $uMemberId; ?>><br>
														<input class="form-control placeholder-no-fix" type="text" placeholder="Name" name="firstName" required autofocus pattern="[A-Z a-z]{3,}" value=<?php echo $result['FirstName']; ?>><br>
							
														<input class="form-control placeholder-no-fix" type="text" placeholder="Year" name="year" value=<?php echo $result['year']; ?>><br>
														<input class="btn btn-primary btn-lg btn-block" type="submit" name="updateMemberBtn" value="Update"><br>
													</form>
												</div>
											...
											if($activity == 'deleteBook'){
												...
												$result = mysqli_num_rows(mysqli_query($db,"SELECT bookId FROM borrow Where bookId = '$deleteBookId'"));
											    $result1 = mysqli_num_rows(mysqli_query($db,"SELECT bookId FROM books Where bookId = '$deleteBookId'"));
												
												if(empty($result)){
												$deleteResult = mysqli_query($db,"Delete From books Where bookId = '$deleteBookId'");
													if(empty($result1)){$errorMsg ="Please Enter valid AccessId";}else{$errorMsg = "$deleteBookId Book Was Sucessfully deleted";}
											...
											if($activity == 'deleteMember'){
												...
												$result = mysqli_num_rows(mysqli_query($db,"SELECT issueId FROM borrow Where issueId = '$deleteMemberId'"));
											    $result1 = mysqli_num_rows(mysqli_query($db,"SELECT Id FROM faculty Where Id = '$deleteMemberId'"));
												if(empty($result)){
													$deleteResult = mysqli_query($db,"Delete From faculty Where Id = '$deleteMemberId'");
													if(empty($result1)){$errorMsg ="Please Enter valid FacultyId";}else{$errorMsg = "Faculty Record Was Sucessfully deleted";}
											...
											if($activity == 'deleteReturnedBooksHistory'){
												...
												$deleteResult = mysqli_query($db,"Delete From borrow Where returnId = '$deleteReturnId' && returnDate = '$deleteReturnDate'");
									...
									// CODE FOR UPDATE MEMBER...
										...
										if(isset($_REQUEST['updateMemberBtn'])){
											...
											$query1 = mysqli_query($db,"UPDATE members Set FirstName='$firstName', year='$year' Where Id = '$memberId'");
